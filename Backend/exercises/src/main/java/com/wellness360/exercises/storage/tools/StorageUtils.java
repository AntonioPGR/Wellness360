...
import com.wellness360.exercises.storage.validations.custom.StorageFileNotFoundException;
...
public class StorageUtils {
  ...
  public static void checkDestinationPathIsInRoot(Path destination_file, Path root_location){
    ...
    if(!destination_path.contains(root_path)){
      ErrorsThrower.storageError("Cannot store file outside current directory");
    }
  }

  public static void storageFile(MultipartFile file, Path destination_path){
    try {
